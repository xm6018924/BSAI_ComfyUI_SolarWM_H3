...
class BSAI_SolarWM_H3_Loader:
    ...
    def load(self, base_model, turbo_lora, turbo_strength,
        ...
        model = comfy.sd.load_diffusion_model(
            folder_paths.get_full_path_or_raise("diffusion_models", base_model),
            model_options=model_options,
        )

        turbo_path = folder_paths.get_full_path_or_raise("loras", turbo_lora)
        if turbo_strength > 0:
            lora = comfy.utils.load_torch_file(turbo_path, safe_load=True)
            model, _ = comfy.sd.load_lora_for_models(model, None, lora, turbo_strength, 0.0)
            del lora

        solarwm_path = folder_paths.get_full_path_or_raise("loras", solarwm_lora)
        if solarwm_strength > 0:
            lora = comfy.utils.load_torch_file(solarwm_path, safe_load=True)
            model, _ = comfy.sd.load_lora_for_models(model, None, lora, solarwm_strength, 0.0)
            del lora
